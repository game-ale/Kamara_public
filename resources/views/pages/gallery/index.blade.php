    ...
    <section class="py-24 bg-slate-50 min-h-[50vh]">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                <a href="/gallery/{{ $gallery->slug }}" class="group relative bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 block aspect-[4/3]">
                    @php
                        $coverPath = $gallery->cover_image ?? ($gallery->images[0] ?? null);
                    @endphp
                    @if($coverPath)
                        <img src="{{ route('media.proxy', ['path' => $coverPath]) }}" alt="{{ $gallery->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    @else
